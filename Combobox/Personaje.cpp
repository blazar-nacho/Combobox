...
void Personaje::reiniciarAtributos(){
	vida = MAX_VIDA;
}


void Personaje::setVida(int unaVida){
	if (unaVida == VIDA_INFINITA){
		vida = MAX_VIDA;
		vidaInfinita = true;
	}
}
...
void Personaje::copiarAtributosDe(Personaje* unPersonaje){
	...
	fatalityDir = unPersonaje->getFatalityDir();
	sonidosDir = unPersonaje->getSonidosDir();
	if (sonidos != nullptr) delete sonidos;
	sonidos = new Sonido(sonidosDir);
	spritesDir = unPersonaje->getSpriteDir();
...
Personaje::Personaje(float anchoPersonaje, float altoPersonaje, int zIndexPersonaje, std::string unaorientacion, std::string spritesDirPersonaje, std::string CaminarParaAdelante, std::string CaminarParaAtras, std::string QuietoPersonaje, std::string SaltoPersonaje, std::string SaltoDiagonalPersonaje, std::string CaidaPersonaje, std::string PatadaAltaPersonaje, std::string GolpeadoPersonaje, std::string AgachadoPersonaje, std::string nombrePersonaje, std::string sonidosDirPersonaje, std::string fatalityDirPersonaje)
{
	...
	vida = MAX_VIDA;
	fatalityDir = fatalityDirPersonaje;
	sonidosDir = sonidosDirPersonaje;
	sonidos = new Sonido(sonidosDir);
...
std::string Personaje::getFatalityDir() const
{
	return fatalityDir;
}
...
std::string Personaje::getSonidosDir() const
{
	return sonidosDir;
}

Sonido* Personaje::getSonidos() const
{
	return sonidos;
}
...
int Personaje::descontarVida(ESTADO estadoPj, ESTADO estadoPj2)
{
	if (!vidaInfinita){
		...
		if ((vida <= 0) && MODO_INMORTAL) vida = MAX_VIDA; //XJOSE esto esta para poder probar el juego tranqujilo
		if (vida <= 0)
			return REINICIAR;
	}
	return CONTINUAR;
...
std::string Personaje::getNombreActual() const{
	return nombreActual;
}
...
void Personaje::setSprites(std::string unosSprites){
	...
	sprites = new Sprite(spritesDir);
	// los sprites nuevos cargas sus sensores, así que los redimensiona
	ManejadorULogicas manejador;
...
void Personaje::setFatality(std::string unaFatality)
{
	fatalityDir = unaFatality;
}


void Personaje::setSonidos(std::string unosSonidos) {
	// borra sonidos anteriores
	delete sonidos;
	// carga y asigna los sonidos nuevos
	sonidosDir = unosSonidos;
	sonidos = new Sonido(sonidosDir);	
}
...
void Personaje::setNombreActual(std::string unNombre){
	nombreActual = unNombre;
}
...
void Personaje::actualizar(float xNuevo, float yNuevo, ESTADO nuevoEstado)
{
	...
	sonidos->reproducirSonido(nuevoEstado);
}
