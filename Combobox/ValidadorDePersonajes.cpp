...
bool ValidadorDePersonajes::validarPersonajesDesdeConEscenarioYventana(Json::Value personajesParseados, Escenario* unEscenario, Ventana* unaVentana){
	...
	if (!personajesParseados || personajesParseados.size() == 0){
		...
		personajes->push_back(new Personaje(ANCHO_PERSONAJE, ALTO_PERSONAJE, ZINDEX, ORIENTACION_PERSONAJE, SPRITE_DEFAULT, CAMINARPARAADELANTE_DEFAULT, CAMINARPARAATRAS_DEFAULT, QUIETO_DEFAULT, SALTO_DEFAULT, SALTODIAGONAL_DEFAULT, CAIDA_DEFAULT, PATADAALTA_DEFAULT, GOLPEADO_DEFAULT, AGACHARSE_DEFAULT, LIU_KANG, SONIDOS_DEFAULT, FATALITY_DEFAULT));
		personajes->push_back(new Personaje(ANCHO_PERSONAJE, ALTO_PERSONAJE, ZINDEX, ORIENTACION_PERSONAJE, SPRITE_DEFAULT, CAMINARPARAADELANTE_DEFAULT, CAMINARPARAATRAS_DEFAULT, QUIETO_DEFAULT, SALTO_DEFAULT, SALTODIAGONAL_DEFAULT, CAIDA_DEFAULT, PATADAALTA_DEFAULT, GOLPEADO_DEFAULT, AGACHARSE_DEFAULT, LIU_KANG, SONIDOS_DEFAULT, FATALITY_DEFAULT));
		return true;
	}
	else{
		...
		for (size_t i = 0; i < personajesParseados.size(); i++) {
			...
			Personaje* unPersonaje = new Personaje(ANCHO_PERSONAJE, ALTO_PERSONAJE, ZINDEX, ORIENTACION_PERSONAJE, SPRITE_DEFAULT, CAMINARPARAADELANTE_DEFAULT, CAMINARPARAATRAS_DEFAULT, QUIETO_DEFAULT, SALTO_DEFAULT, SALTODIAGONAL_DEFAULT, CAIDA_DEFAULT, PATADAALTA_DEFAULT, GOLPEADO_DEFAULT, AGACHARSE_DEFAULT, LIU_KANG, SONIDOS_DEFAULT, FATALITY_DEFAULT);
			...
			//SONIDOS
			if (personajesParseados[i].isMember("sonidos") && personajesParseados[i].get("sonidos", SONIDOS_DEFAULT).isString()){
				sonidos = (personajesParseados[i].get("sonidos", SONIDOS_DEFAULT).asString());

				archivoPersonaje = sonidos.c_str();
				fopen_s(&pFile, archivoPersonaje, "r");

				if (pFile != NULL) //si me devolvio puntero existe, cerralo!!!!!
				{
					fclose(pFile);
					unPersonaje->setSonidos(sonidos);
				}
				else // el archivo no existe!
				{
					std::string mensaje = "sonidos no existentes";
					Log::getInstancia().logearMensajeEnModo(mensaje, Log::MODO_WARNING);
					sonidos = SONIDOS_DEFAULT;
					archivoPersonaje = sonidos.c_str();
					fopen_s(&pFile, archivoPersonaje, "r");

					if (pFile == NULL) //si me devolvio puntero existe, cerralo!!!!!
					{
						Log::getInstancia().logearMensajeEnModo("sonidos por defecto no existentes, se cerrara el programa...", Log::MODO_ERROR);
						return false;
				...
				Log::getInstancia().logearMensajeEnModo("Se cargan sonidos del personaje por defecto", Log::MODO_WARNING);
			}


			//FATALITY
			if (personajesParseados[i].isMember("fatality") && personajesParseados[i].get("fatality", SONIDOS_DEFAULT).isString()){
				fatality = (personajesParseados[i].get("fatality", SONIDOS_DEFAULT).asString());

				archivoPersonaje = fatality.c_str();
				fopen_s(&pFile, archivoPersonaje, "r");

				if (pFile != NULL) //si me devolvio puntero existe, cerralo!!!!!
				{
					fclose(pFile);
					unPersonaje->setFatality(fatality);
				}
				else // el archivo no existe!
				{
					std::string mensaje = "fatality no existente";
					Log::getInstancia().logearMensajeEnModo(mensaje, Log::MODO_WARNING);
					fatality = SONIDOS_DEFAULT;
					archivoPersonaje = fatality.c_str();
					fopen_s(&pFile, archivoPersonaje, "r");

					if (pFile == NULL) //si me devolvio puntero existe, cerralo!!!!!
					{
						Log::getInstancia().logearMensajeEnModo("fatality por defecto no existente, se cerrara el programa...", Log::MODO_ERROR);
						return false;
				...
				Log::getInstancia().logearMensajeEnModo("Se carga fatality del personaje por defecto", Log::MODO_WARNING);
			}
