...
void ValidadorDelControlador::validarControladorDesde(Json::Value unControlador){
	...
	if (!unControlador){
		...
		if ((unControlador.isMember("reiniciar_pelea") && unControlador.get("reiniciar_pelea", REINICIAR_PELEA_DEFAULT).isString())){
			reiniciarPelea = unControlador.get("reiniciar_pelea", REINICIAR_PELEA_DEFAULT).asString();
		}
		else{
