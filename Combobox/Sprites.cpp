...
Sprite::Sprite(std::string jsonSprites){
	...
	// Sensores Cayendo
	cargarSensores("Cayendo", sprites);

	// Sprites Tumbado
	cargarSprites(Tumbado, "Tumbado", sprites);
	// Sensores Tumbado
...
void Sprite::redimensionarSensores(float anchoPjPx, float altoPjPx)
{
	...
	sensoresDisparo.at(0)->setPosicion(std::make_pair(anchoPjPx / 3, altoPjPx / 2.5));
	sensoresDisparo.at(0)->setPosicionInicial(std::make_pair(anchoPjPx / 3, altoPjPx / 2.5));
...
void Sprite::setFatality(std::vector<SDL_Rect*>* FatalityNueva)
{
	Fatality = FatalityNueva;
}
...
Sprite::~Sprite()
{
	...
	for (size_t i = 0; i < Tumbado->size(); i++)
		delete Tumbado->at(i);
	Tumbado->clear();
	delete Tumbado;
