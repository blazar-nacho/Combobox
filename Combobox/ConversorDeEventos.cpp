...
ConversorDeEventos::CodigosDeEvento ConversorDeEventos::getCodigosDeEventoDeTecla(std::string tecla){
	...
	codigosDeEvento.ejeDesde = 0;
	codigosDeEvento.ejeHasta = 0;
...
std::string ConversorDeEventos::getSimboloDeLaAccion(Acciones unaAccion){
	...
	if (acciones[unaAccion].eje){
		if (acciones[unaAccion].jBoton == 0){
			if (acciones[unaAccion].ejeDesde > 0) return "Right";
			return "Left";
		}
		if (acciones[unaAccion].ejeDesde > 0) return "Down";
		return "Up";
	}
	//FLECHAS
	if (acciones[unaAccion].flechas){
		if (acciones[unaAccion].hat == SDL_HAT_DOWN) return "Down";
		if (acciones[unaAccion].hat == SDL_HAT_UP) return "Up";
		if (acciones[unaAccion].hat == SDL_HAT_LEFT) return "Left";
		if (acciones[unaAccion].hat == SDL_HAT_RIGHT) return "Right";
	}
	//BOTONES
